...
// Detect if query looks like a title search rather than a description
function looksLikeTitle(q: string): boolean {
  const lower = q.toLowerCase().trim();
  // Short query with no descriptive words = likely a title
  const descriptiveWords = /\b(something|show|anime|movie|want|like|feel|mood|genre|episode|season|make|cry|sad|happy|scary|funny|chill|hype|dark|light|old|new|recent|classic)\b/;
  const isShort = q.trim().split(/\s+/).length <= 5;
  const hasNoDescriptive = !descriptiveWords.test(lower);
  return isShort && hasNoDescriptive;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Title search shortcut — skip Groq entirely
    if (looksLikeTitle(query)) {
      return NextResponse.json({
        title_search: query.trim(),
        genres: [],
        mood: null,
        era: "any",
        min_rating: 5.0,
...
Rules:
- genres: 1-3 genres from the valid list. Never empty.
- mood: map "sad/cry" → cry, "scary/horror/spooky" → spooky, "exciting/fight" → hype, "love/romance" → romance, "calm/cozy" → chill
...
JSON: {"genres": [...], "mood": "...", "era": "...", "min_rating": 6.0}`,
    ...
    if (genres.length === 0 && mood) genres = MOOD_GENRE_MAP[mood] ?? ["Drama"];
    ...
    const min_rating = Math.min(Math.max(
      typeof parsed.min_rating === "number" ? parsed.min_rating : 6.0, 0), 6.5);

    return NextResponse.json({ genres, mood, era, min_rating, title_search: null });

  } catch (error) {
    console.error("parse-query error:", error);
    return NextResponse.json(
      { genres: ["Drama"], mood: null, era: "any", min_rating: 6.0, title_search: null },
      { status: 500 }
    );
  }
}
